...
class Vocabulary():
    ...
    def rev_translate(self, words_id : torch.tensor) -> List[str]:
        ...
        """
        if isinstance(words_id, torch.Tensor):
            words_id = words_id.tolist()  # Nếu là tensor thì chuyển thành list

        return [list(self.word2id.keys())[idx] for idx in words_id]   # word_id (1,caption_length)
