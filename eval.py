...
def parse_command_line_arguments():
    ...
    parser.add_argument('--output_csv', type=str, default='eval_caption.csv',
                        help='Output CSV file to save evaluation results (default: "eval_caption.csv")')
...
if __name__ == "__main__":
    ...
    if args.mode == "eval":
        ...
        if args.image_path:
            ...
            # Nếu không chỉ định --image_path, duyệt toàn bộ ảnh trong dataset/images
            image_folder = os.path.join(args.dataset_folder, "images")
    ...
    for image_file in image_files:
        ...
        comment = net.eval_image_caption(image, vocabulary)
        
        if comment[0] == "<START>":
            comment = comment[1:]
            
        if comment and comment[-1] == "<END>":
            comment = comment[:-1]
        caption_text = " ".join(comment).strip()
    ...
    with open(args.output_csv, 'r', encoding='utf-8') as file:
